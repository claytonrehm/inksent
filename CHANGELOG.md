...
- **A2P 10DLC fix — voluntary SMS opt-in + consent gating (Twilio error 30923).** The carrier rejected the messaging campaign because `/apply` couldn't be submitted without checking the SMS-consent box (forced consent). Fixed across both opt-in pages and the send paths so consent is genuinely voluntary AND honored:
  ...
  - **`/order` (client):** SMS consent split out of the **required** ToS clickwrap into its own **optional** checkbox; the order goes through unchecked (updates by email + live tracker instead). Stored as `orders.sms_consent_at` (best-effort, migration-safe).
  - **Consent gating:** notary job-offer SMS (`lib/dispatch.ts` auto-blast, `/api/dispatch`, `/api/dispatch/blast`) and the three admin notary nudges (availability, onboarding, hub announcement) now only text when `notaries.sms_consent_at` is set. Client/signer SMS (`progress` → client, `reschedule` → signer) only send when `orders.sms_consent_at` is set. **Email always sends regardless** — so reach is unchanged for the opted-in.
  - **Backfill (migration `20260629120000_sms_consent_gating.sql`):** adds `orders.sms_consent_at`; stamps existing orders and the existing active notary bench as consented (they opted in under the prior required-checkbox forms), so nothing in flight loses its texts. New applicants who leave the box unchecked stay email-only.
- **Self-service notary availability + nudge.** Availability used to be captured only on the public application form — there was no way for an existing notary to set or change it, so anyone added without it sat permanently in the `/coverage` "haven't set availability yet" bucket and got skipped for dispatch matching. New no-login one-tap page at **`/availability/[id]`** (the unguessable notary UUID is the capability token, same pattern as accept/decline/onboard) with big toggle buttons for the four coarse windows (weekday daytime, weekday evenings, weekends, same-day/last-minute); pre-fills any existing selection and saves via `POST /api/notaries/[id]/availability` (server-side validates against the fixed window set). New admin-gated **`/api/admin/nudge-availability`** finds every active, non-denied notary with an empty `availability` array (exactly the `/coverage` count) and texts + emails them the link, stamping `availability_nudged_at`. New `sendAvailabilityRequestEmail()`. Migration `20260629000000_availability_nudge.sql` (adds `availability_nudged_at`).
