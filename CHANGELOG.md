...
- **A2P 10DLC fix — voluntary SMS opt-in (Twilio error 30923).** The carrier rejected the messaging campaign because `/apply` could not be submitted without checking the SMS-consent box (forced consent). The notary application's SMS consent is now **optional** (`sms_consent` schema rule relaxed from `literal(true)` to `boolean().optional()`); the box is clearly labeled optional and the form submits whether or not it's ticked. The backend already stored consent as `sms_consent_at` (null when not given), so unconsented applicants are simply never texted. **Note:** the `/order` client clickwrap still bundles SMS consent into the required terms acceptance (same flaw, not yet cited) and notary job-offer SMS does not yet gate on `sms_consent_at` — recommended follow-ups before resubmitting the campaign.
